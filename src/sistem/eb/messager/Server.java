...
public class Server implements Runnable {
    ...
    public void run() {
        try {
            ...
            while (true) {
                ...
                // Welcome message
                newUser.getPrintStream().println(
    ...
    public void broadcastMessages(String message, User userSender) {
        for (User client : this.clients) {
            client.getPrintStream().println(userSender.toString() + "<span>: " + message + "</span>");
        }
    }
    
    // send message to a User (String)
    public void sendMessageToUser(String message, User userSender, String user) {
        boolean find = false;
        for (User client : this.clients) {
            if (client.getNickname().equals(user) && client != userSender) {
                find = true;
                userSender.getPrintStream().println(userSender.toString() + " to " + client.toString() + ": " + message);
                client.getPrintStream().println("(<b>Private</b>)" + userSender.toString() + "<span>: " + message + "</span>");
            }
        }
        if (!find) {
            userSender.getPrintStream().println(userSender.toString() + " to (<b>no one!</b>): " + message);
        }
    }
    // send list of clients to all Users
    public void broadcastAllUsers() {
        for (User client : this.clients) {
            client.getPrintStream().println(this.clients);
        }
    }
    ...
    public void run() {
        ...
        while (sc.hasNextLine()) {
            ...
            if (message.charAt(0) == '@') {
                ...
                // update colour for all other users
                this.server.broadcastAllUsers();
            } else {
                // update user list
...
class User {
    ...
    public User(Socket client, String name) throws IOException {
        ...
        this.nickname = name;
        this.userId = nbUser;
        this.colour = ColorInt.getColor(this.userId);
        nbUser += 1;
    }
    ...
    public void changeColor(String hexColor) {
        ...
        if (m.matches()) {
            ...
            if (luma > 160) {
                this.getPrintStream().println("<b>Color too bright, try again!</b>");
                return;
            }
            this.colour = hexColor;
            this.getPrintStream().println("<b>Color changed successfully.</b> " + this.toString());
            return;
        }
        this.getPrintStream().println("<b>Failed to change color.</b>");
    }
    ...
    // print user with his colour
    @Override
    public String toString() {

        return "<u><span style='color:" + this.colour
